...
Attempt to detect what terminal and launcher are actually installed. Ask 
when more than one candidate is installed and there's a terminal to ask on.

Nothing here imports the rest of bwm, so it can run before the config is loaded.
...
# bwedit.py runs the terminal as `<terminal> -e <editor> <file>` and waits for
...
# needs, shown when asking. The pynput default types nothing at all on Wayland,
# which is the most common "autotype does nothing" report.
...
def has_tty():
    ...
    collecting --show output. Usually there isn't: bwm is normally started from a keybinding.
...
def detect_type_library(interactive=False):
    ...
    """
    return pick("Which autotype backend should bwm use?",
...
def detect(interactive=False):
    ...
    if interactive and has_tty() and any(
        ...
        say("Setting up bwm. Press enter to take the suggested answer.")
    return {
        "launcher": pick("Which launcher should bwm use?",
                         launchers, interactive),
        "terminal": pick("Which terminal should bwm open editors in?",
...
def bw_cli_missing_msg():
    """Message for a system without the Bitwarden CLI, which bwm drives.

    Returns: str, or None when `bw` is installed

    """
    if which("bw") is not None:
        return None
    return ("The Bitwarden CLI (bw) is not installed. bwm runs every vault "
            "operation through it.\nInstall it from "
            "https://bitwarden.com/help/cli/ or your distribution's packages.")
